...
class Sessions extends Models implements OCREND {
  ...
  */
  public function generate_session(int $id) {
    ...
    $this->db->update('users',$e,"Id_user='$id'",'LIMIT 1');
  }
  ...
  */
  public function session_in_use(int $id) : bool {
    $time = time();
    if($this->db->rows($this->db->query("SELECT Id_user FROM users WHERE Id_user='$id' AND session >= '$time' LIMIT 1;")) > 0) {
      return true;
    }
  ...
  */
  public function check_life(bool $force = false) {
    if(isset($_SESSION[SESS_APP_ID])) {
      ...
      if($force || $this->db->rows($this->db->query("SELECT Id_user FROM users WHERE Id_user='$id' AND session <= '$time' LIMIT 1;")) > 0) {
